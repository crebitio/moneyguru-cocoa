...
def clean():
    TOCLEAN = [
        ...
        'dist',
        'install',
        op.join('moneyguru', 'help', 'en', 'image')
    ]
    for path in TOCLEAN:
        try:
            os.remove(path)
...
def build_cocoa(dev):
    ...
    resources = [
        'cocoa/dsa_pub.pem', 'build/mg_cocoa.py', 'build/help', 'moneyguru/data/example.moneyguru',
    ] + glob.glob('images/*')
...
def build_help():
    ...
    current_path = op.abspath('moneyguru')
...
def build_mergepot():
    ...
    loc.merge_pots_into_pos('cocoalib/locale')
